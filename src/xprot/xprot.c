...
void
xprot_process_stereo_srcdst(xprot *xp, int16_t *src_dst_left,
                            int16_t *src_dst_right, int16_t length)
{
  if (length > 0)
  {
    ...
    do
    {
      ...
      {
        a_xprot_func_s(xp->xprot_left_variable, xp->xprot_left_fixed,
                       xp->xprot_right_variable, xp->xprot_right_fixed,
                       src_dst_left, src_dst_right, xp->temp_limit,
                       xp->displ_limit);
      ...
      a_xprot_func(xp->xprot_left_variable, xp->xprot_left_fixed, src_dst_left,
                   xp->temp_limit, xp->displ_limit);

      if (xp->stereo == 3)
        a_xprot_func(xp->xprot_right_variable, xp->xprot_right_fixed,
                     src_dst_right, xp->temp_limit, xp->displ_limit);
...
void
xprot_process_stereo(xprot *xp, int16_t *src_left, int16_t *src_right,
                     int16_t *dst_left, int16_t *dst_right, int16_t length)
{
...
void
xprot_change_params(xprot *xp, const void *parameters, size_t length,
                    int channel)
{
  xprot_channel *c;
  ...
  memcpy(c->constant, parameters, length);
...
xprot *xprot_new(void)
{
  ...
  if (xp->xprot_left_const)
  {
    xp->xprot_left_variable =
        (XPROT_Variable *) pa_xmalloc(sizeof(XPROT_Variable));
    ...
    if (xp->xprot_left_variable)
    {
      ...
      {
        xp->xprot_right_const =
            (XPROT_Constant *) pa_xmalloc(sizeof(XPROT_Constant));
        ...
        if (xp->xprot_right_const)
        {
          ...
          if (xp->xprot_right_variable)
          {
            xp->xprot_right_fixed =
                (XPROT_Fixed *) pa_xmalloc(sizeof(XPROT_Fixed));

            if (xp->xprot_right_fixed)
            {
              xp->xprot_left_channel =
                  (xprot_channel *) pa_xmalloc(sizeof(xprot_channel));

              if (xp->xprot_left_channel)
              {
                xp->xprot_right_channel =
                    (xprot_channel *) pa_xmalloc(sizeof(xprot_channel));
